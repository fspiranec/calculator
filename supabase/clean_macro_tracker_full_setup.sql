-- Clean Macro Tracker complete Supabase setup.
-- Paste this whole RAW file into Supabase SQL Editor.
-- It is safe to re-run: tables/columns are created if missing, policies are dropped/recreated.
...
alter table public.user_profile_settings add column if not exists calorie_adjustment integer;
...
begin
  foreach table_name in array array['profiles','user_goals','user_profile_settings','custom_foods','food_entries','weight_entries','predefined_foods'] loop
    ...
    if table_name <> 'predefined_foods' then
      execute format('create trigger set_%I_updated_at before update on public.%I for each row execute function public.set_updated_at()', table_name, table_name);
    end if;
  end loop;
end $$;

-- Recreate policies so this script can be re-run.
...
drop policy if exists "profile_settings select own" on public.user_profile_settings;
drop policy if exists "profile_settings insert own" on public.user_profile_settings;
drop policy if exists "profile_settings update own" on public.user_profile_settings;
drop policy if exists "profile_settings delete own" on public.user_profile_settings;
...
drop policy if exists "custom_foods select own" on public.custom_foods;
drop policy if exists "custom_foods insert own" on public.custom_foods;
drop policy if exists "custom_foods update own" on public.custom_foods;
drop policy if exists "custom_foods delete own" on public.custom_foods;
...
drop policy if exists "food_entries select own" on public.food_entries;
drop policy if exists "food_entries insert own" on public.food_entries;
drop policy if exists "food_entries update own" on public.food_entries;
drop policy if exists "food_entries delete own" on public.food_entries;
...
drop policy if exists "weight_entries select own" on public.weight_entries;
drop policy if exists "weight_entries insert own" on public.weight_entries;
drop policy if exists "weight_entries update own" on public.weight_entries;
drop policy if exists "weight_entries delete own" on public.weight_entries;
...
drop policy if exists "predefined foods readable" on public.predefined_foods;
create policy "predefined foods readable" on public.predefined_foods for select to authenticated using (true);

-- Optional profile auto-create on signup.
