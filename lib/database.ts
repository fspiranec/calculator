...
const fromProfile = (row: any): UserProfile => row ? { sex: row.sex ?? undefined, age: row.age ?? undefined, heightCm: row.height_cm ? Number(row.height_cm) : undefined, activityLevel: row.activity_level ?? undefined, goal: row.goal ?? row.fitness_goal ?? undefined, updatedAt: row.updated_at } : {};
const toProfile = (userId: string, profile: UserProfile) => ({ user_id: userId, sex: profile.sex ?? null, age: profile.age ?? null, height_cm: profile.heightCm ?? null, activity_level: profile.activityLevel ?? null, goal: profile.goal ?? null });
